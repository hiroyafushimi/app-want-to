...
各セクションには依存関係があるため、以下の順序で進めてください。

```
Phase 1: ストア側の準備（Apple / Google）
  ...
  └── B. Google Play Console（Android） … セクション 2
        ...
        ├─ 2-3. アプリ内アイテムを作成
        ├─ 2-4. Service Account JSON を取得  ← RevenueCat 登録に必要
        └─ 2-5. ライセンステスト用アカウント設定
...
Phase 3: ビルド & テスト               … セクション 4
  ├─ 4-1. API Key でビルド
  ├─ 4-2. Sandbox / ライセンステスト
  └─ 4-3. リリースビルド
...
1. [App Store Connect の設定（iOS）](#1-app-store-connect-の設定ios)
2. [Google Play Console の設定（Android）](#2-google-play-console-の設定android)
3. [RevenueCat ダッシュボード設定](#3-revenuecat-ダッシュボード設定)
4. [ビルドと API Key の渡し方](#4-ビルドと-api-key-の渡し方)
5. [アプリアイコン](#5-アプリアイコン)
...
## 1. App Store Connect の設定（iOS）

> **このセクションを先に完了する。** ここで取得する Shared Secret は、後のセクション 3（RevenueCat）で必要になる。
...
> **この値をセクション 3-2 で使う。** コピーしたらメモしておくこと。
...
### 2-3. アプリ内アイテム（買い切り商品）を作成

> **前提**: アプリ内アイテムを作成するには、先にアプリの **App Bundle を 1 回以上アップロード** する必要がある（内部テストトラックでOK）。先に [2-6](#2-6-内部テストでアプリをアップロード) を完了させること。
...
### 2-4. Service Account JSON を取得（RevenueCat 連携用）

> **この JSON ファイルをセクション 3-3 で使う。** ダウンロードしたら安全な場所に保存しておくこと。
...
### 2-6. 内部テストでアプリをアップロード

> **2-3 でアプリ内アイテムを作成する前に、ここを先に実行する。** 課金商品を有効化するには最低 1 回 App Bundle をアップロードする必要がある。
...
2. ダッシュボード左上の **「+ Create new project」** をクリック
3. プロジェクト名: **ActClip** と入力して作成

### 3-2. App（iOS）を登録

1. 左メニュー **「Apps & providers」** をクリック
   - 見つからない場合: **Settings** ページではなくプロジェクトのメインダッシュボード（`/projects/{プロジェクトID}`）の左サイドバーを確認
2. **「+ New」** または **「Add app config」** をクリック
...
3. **Service Credentials JSON**: セクション 2-4 でダウンロードした JSON ファイルをアップロード
...
1. 左メニュー **「Product catalog」** → **「Products」** → **「+ New」**
...
1. 左メニュー **「Product catalog」** → **「Offerings」** → デフォルトの `default` Offering をクリック（なければ「+ New」）
2. **「+ New Package」** をクリック
...
### 4-2. VS Code / Cursor の launch.json に設定（毎回入力を省略）
